...
def test_three_dim_spec_raise_error() -> None:
  ...
  with pytest.raises(ValueError):
    compare_amplitude_spectrograms(
      np.empty((305, 20, 10)), np.empty((305, 20, 10)), SR, N_FFT
    )
...
def test_empty_spec_returns_nan_nan() -> None:
  ...
  mcd, pen = compare_amplitude_spectrograms(
    get_X_km_A(), X_km_empty, 22050, samples_to_ms(512, 22050)
  )
  assert np.isnan(mcd)
  assert np.isnan(pen)

  mcd, pen = compare_amplitude_spectrograms(
    X_km_empty, X_km_empty, 22050, samples_to_ms(512, 22050)
  )
  assert np.isnan(mcd)
  assert np.isnan(pen)
...
def test_invalid_n_fft_raises_error() -> None:
  ...
  with pytest.raises(ValueError):
    compare_amplitude_spectrograms(
      get_X_km_A(), get_X_km_B(), 22050, samples_to_ms(511, 22050)
    )
...
def test_invalid_D_raises_error() -> None:
  ...
  with pytest.raises(ValueError):
    compare_amplitude_spectrograms(
      get_X_km_A(), get_X_km_B(), 22050, samples_to_ms(512, 22050), D=1
    )
