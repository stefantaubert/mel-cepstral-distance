...
def test_fill_rest_with_zeros() -> None:
  ...
  assert padded_mfccs_1.shape == padded_mfccs_2.shape, (
    "Arrays should have the same shape after padding"
  )

  # Check if the original values are unchanged
  assert np.array_equal(padded_mfccs_1[:, :3], mfccs_1), (
    "Original values in mfccs_1 should remain unchanged"
  )
  assert np.array_equal(padded_mfccs_2[:, :2], mfccs_2), (
    "Original values in mfccs_2 should remain unchanged"
  )
