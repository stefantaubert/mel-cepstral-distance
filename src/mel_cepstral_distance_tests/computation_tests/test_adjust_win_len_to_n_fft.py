...
def test_padding_short_window() -> None:
...
def test_truncating_long_window() -> None:
  ...
  assert truncated_frames.shape == (3, n_fft), (
    "Frames should be truncated to match n_fft."
  )
...
def test_no_change_when_equal() -> None:
  ...
  assert np.array_equal(unchanged_frames, windowed_frames), (
    "Frames should remain unchanged when win_len == n_fft."
  )
  assert unchanged_frames.shape == (3, n_fft), "Shape should remain the same."
  assert win_len == n_fft, "win_len should remain unchanged."


def test_edge_case_empty_frames() -> None:
  """Handles empty input frames correctly."""
  windowed_frames = np.empty((0, 10))  # No frames, 10 Samples
  n_fft = 16
  padded_frames, win_len = adjust_win_len_to_n_fft(windowed_frames, n_fft)

  assert padded_frames.shape == (0, n_fft), (
    "Empty input should still return the correct shape."
  )
  assert win_len == n_fft, "win_len should be set to n_fft."


def test_edge_case_single_frame() -> None:
